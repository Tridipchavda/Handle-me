...
function getLabelColor(label: string) {
  return (
    LABEL_OPTIONS.find((option) => option.value === label)?.color ||
...
export default function DashboardPage() {
  ...
  const togglePin = async (taskId: string, currentPinState: boolean) => {
    try {
      const updatedTask = await taskStorage.updateTask(taskId, {
          ...
          <div className="animate-pulse">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="bg-white p-6 rounded-lg shadow-sm h-48"
                ></div>
              ))}
            </div>
          </div>
                ...
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className={`p-1 rounded-full text-red-500 hover:text-white hover:bg-red-500 transition-colors duration-150 ${
